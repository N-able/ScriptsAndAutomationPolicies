<#    
   *******************************************************************************************************************************
    Name:            Repair-PME.ps1
    Version:         0.1.7.0 Beta (12/06/2020)
    ...
    Version History: 0.1.0.0 - Initial Release.
                     ...
                     0.1.6.5 - New function 'Set-PMEConfig' to apply fix for NCPM-4407 (System.OutOfMemoryException)
                               This will be applied by default however if you wish to change this behaviour change variable 
                ...
                0.1.7.0 Beta - New function 'Confirm-PMEInstalled' to confirm if PME is already installed.
...
Function Confirm-PMEUpdatePending {
    ...
    # Change $Days variable for when to begin repair after new version of PME released
    $Days = "2"
    ...
    If (($IsPMEInstalled -eq "Yes") -and ($PMEWrapper -ne "") -and ($PMEWrapper -match $NewVersion) -and ($PMEWrapper -match $LastInstall)) {
        ...
        Write-Host "INFO: Last PME check detected version ($LastInstallDateVersion) and installed it on ($LastInstallDate)" -ForegroundColor Cyan
        $ConvertPMEConfigurationDate = Get-Date "$PMEConfigurationDate"
        $SelfHealingDate = $ConvertPMEConfigurationDate.AddDays($Days).ToString('yyyy.MM.dd')
        Write-Host "INFO: Repair-PME will only proceed ($Days) days after a new version of PME has been released" -ForegroundColor Cyan
        ...
        Else {
        Write-EventLog -LogName Application -Source "Repair-PME" -EntryType Information -EventID 100 -Message "Error occurred attempting to obtain PMESetup details, aborting. Error: $($_.Exception.Message).`nScript: Repair-PME.ps1"
        Throw "($DaysElapsed) days has elapsed since a new version of PME has been released, aborting."   
        Write-Output "($DaysElapsed) days has elapsed since a new version of PME has been released, aborting"
        }
    }
    Else {
        Write-Host "Skipping update pending check as PMEWrapper.log does not currently contain update information" -ForegroundColor Yellow
    }    
}
...
Function Test-Connectivity {
    ...
    Else {
        Write-Host "Skipping connectivity tests as OS is Windows 7/Server 2008 R2 and/or Powershell 4.0 or above is not installed" -ForegroundColor Yellow
        $Fallback = "Yes"    
    }
}
...
Function Set-PMEConfig {
    ...
    If ($RpcServerServiceConfig -match '<UnloadModuleAppDomainAfterEachRequest>false</UnloadModuleAppDomainAfterEachRequest>') {
        Write-Host "INFO: RpcServerService configuration to address NCPM-4407 (System.OutOfMemoryException) is already applied" -ForegroundColor Cyan
    }    
    ...
    If (($NCPM4407 -eq "Yes") -and ($RpcServerServiceConfig -match '<UnloadModuleAppDomainAfterEachRequest>true</UnloadModuleAppDomainAfterEachRequest>')) {
        Try {
            Write-Output "Changing RpcServerService configuration to address NCPM-4407 (System.OutOfMemoryException)"
        ...
        Catch {
            ...
            Throw "Unable to change RpcServerService configuration to address NCPM-4407 (System.OutOfMemoryException). Error: $($_.Exception.Message)"
        }
    }     
}
